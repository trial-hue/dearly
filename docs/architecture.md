...
- Pricing has one source of truth: `src/domain/constants.ts`. `quote()` (`src/domain/pricing.ts`) turns a size, finish, mode, digital copy, gift and first-card-free flag into lines, totals, ex-VAT, costs and contribution in integer pence, running the arithmetic exact and rounding half up once at the end. `chooseMode()` and `allowedModes()` take the finish because pick-up is Regular Classic or Signature only. `economics()` blends 20% Classic, 60% Signature and 20% Luxe (Regular, advance) for the break-even. Business, florist and postage figures come from the same file. `src/domain/__tests__/no-price-literals.spec.ts` fails the build when a currency amount appears in `src/app`, `src/components` or `src/server`; `docs/pricing.md` is generated from the constants.
