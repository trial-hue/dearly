...
import { AGENT_ACTIONS, agentFallback, type AgentOrderView, type AgentTurnResult } from '@/domain';
...
export const agentTurnJob = defineJob<AgentInput, AgentTurnResult>({
  ...
  buildPrompt(input) {
    return [
      ...
      `Data: ${JSON.stringify({ orders: input.orders, recentTurns: input.turns.slice(-6), message: input.message.slice(0, 1000) })}`,
