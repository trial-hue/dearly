  ...
  return (
    <>
      <PageHeader
        ...
        lede="A florist's order tells us who matters to their customer. Dearly reads it, sets a reminder for next year with the first Regular card free once three dates are saved, and pays the florist a referral fee. Next year the flowers are ordered through Dearly and the florist earns commission."
      >
        <Simulated what="Bloom and Co" />
      </PageHeader>
      <FloristWorkbench sample={sampleFloristOrder(now())} />
      <h2 className="section-title">Ledger</h2>
      <div data-testid="referrals">
        {referrals.length === 0 ? (
          <Empty>No referrals yet. Read the sample order and claim the first-card offer.</Empty>
        ) : (
          <>
                ...
                <tbody>
                  {referrals.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.ledger.recipient} ({r.ledger.occasion.replace('_', ' ')}), first card
                        free with three dates
                      </td>
